...
void initMapInfo(struct Player *player) {
    ...
    for (int i = 0; i < player->row_cnt; i++) {
        for (int j = 0; j < player->col_cnt; j++) {
            ...
            if (cell_type_map[i][j] == SUPER_STAR) {
                int opp_dist = realDistance(player, player->opponent_posx, player->opponent_posy, i, j);
                if (opp_dist < 5) {
                    value_map[i][j] += 100;
                } else {
                    value_map[i][j] += 20;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (x >= 0 && x < player->row_cnt && y >= 0 && y < player->col_cnt) {
            ...
            for (int dx = -GHOST_DANGER_DISTANCE-2; dx <= GHOST_DANGER_DISTANCE+2; dx++) {
                for (int dy = -GHOST_DANGER_DISTANCE-2; dy <= GHOST_DANGER_DISTANCE+2; dy++) {
                    int nx = x + dx;
                    int ny = y + dy;
                    if (nx >= 0 && nx < player->row_cnt && ny >= 0 && ny < player->col_cnt) {
                        int distance = abs(dx) + abs(dy);
                        if (distance <= GHOST_DANGER_DISTANCE+2 && player->your_status <= 0) {
                            value_map[nx][ny] -= (GHOST_DANGER_DISTANCE+2 - distance + 1) * 8;
                        }
                    }
                }
            }
    ...
    if (opp_x >= 0 && opp_x < player->row_cnt && opp_y >= 0 && opp_y < player->col_cnt) {
        ...
        for (int dx = -OPPONENT_DANGER_DISTANCE-2; dx <= OPPONENT_DANGER_DISTANCE+2; dx++) {
            for (int dy = -OPPONENT_DANGER_DISTANCE-2; dy <= OPPONENT_DANGER_DISTANCE+2; dy++) {
                int nx = opp_x + dx;
                int ny = opp_y + dy;
                if (nx >= 0 && nx < player->row_cnt && ny >= 0 && ny < player->col_cnt) {
                    int distance = abs(dx) + abs(dy);
                    if (distance <= OPPONENT_DANGER_DISTANCE+2 && player->opponent_status > 0 && player->your_status == 0) {
                        value_map[nx][ny] -= (OPPONENT_DANGER_DISTANCE+2 - distance + 1) * 12;
                    }
                }
            }
        }
...
bool findPath(struct Player *player, int startX, int startY, int targetX, int targetY, struct Point *path) {
    resetAStar();
    Node startNode = {startX, startY, 0, heuristic(startX, startY, targetX, targetY), heuristic(startX, startY, targetX, targetY), NULL};
    ...
    while (open_count > 0) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (index == -1) {
                Node neighbor = {nx, ny, newG, heuristic(nx, ny, targetX, targetY), newG + heuristic(nx, ny, targetX, targetY), &closed_list[closed_count - 1]};
...
void updateAIState(struct Player *player) {
    ...
    if (player->your_status > 0 && player->opponent_status == 0) {
        int dist = realDistance(player, player->your_posx, player->your_posy, player->opponent_posx, player->opponent_posy);
        if (dist < 6) {
            currentState = STATE_HUNT_GHOST; // 复用猎杀状态
        }
    }
    // 普通状态下，规避强化对手
    if (player->your_status == 0 && player->opponent_status > 0) {
    ...
    switch (currentState) {
        ...
        case STATE_EAT_NORMAL_STAR:
            ...
            for (int g = 0; g < 2; g++) {
                ...
                int ghostY = player->ghost_posy[g];
                if (ghostX >=0 && ghostX < player->row_cnt && ghostY >=0 && ghostY < player->col_cnt) {
                    if (realDistance(player, currentX, currentY, ghostX, ghostY) < 3) {
                        currentState = STATE_AVOID_GHOST;
                        break;
                    }
                }
...
struct Point walk(struct Player *player) {
    ...
    // 非强化状态下，优先远离鬼和强化对手
    if (player->your_status == 0) {
        int bestX = currentX, bestY = currentY, maxDist = -1;
        int dx[] = {-1, 1, 0, 0};
        int dy[] = {0, 0, -1, 1};
        for (int i = 0; i < 4; i++) {
            int nx = currentX + dx[i];
            ...
            if (nx >=0 && nx < player->row_cnt && ny >=0 && ny < player->col_cnt && !isWall(player, nx, ny)) {
                ...
                int oppDist = realDistance(player, nx, ny, player->opponent_posx, player->opponent_posy);
                if (player->opponent_status > 0 && oppDist < minGhostDist) minGhostDist = oppDist;
    ...
    // 对手强化快结束时提前布局抢超级星
    if (player->opponent_status > 0 && player->opponent_status < 5 && player->your_status == 0) {
        struct Point nextSuperStar;
        if (findNearestSuperStar(player, currentX, currentY, &nextSuperStar)) {
            if (realDistance(player, currentX, currentY, nextSuperStar.X, nextSuperStar.Y) < 8) {
                currentState = STATE_EAT_SUPER_STAR;
            }
        }
    }
    ...
    if (player->your_status > 0) {
        if (player->opponent_status == 0) {
            int dist = realDistance(player, currentX, currentY, player->opponent_posx, player->opponent_posy);
            if (dist < 8) {
                int wall_cnt = 0;
                int dx[] = {-1, 1, 0, 0};
                int dy[] = {0, 0, -1, 1};
                for (int d = 0; d < 4; d++) {
                    int nx = player->opponent_posx + dx[d];
                    int ny = player->opponent_posy + dy[d];
                    if (nx < 0 || nx >= player->row_cnt || ny < 0 || ny >= player->col_cnt || isWall(player, nx, ny)) wall_cnt++;
                }
                if (wall_cnt >= 2) {
                    if (findPath(player, currentX, currentY, player->opponent_posx, player->opponent_posy, &nextMove)) {
                        return nextMove;
                    }
                }
            }
    ...
    // 领先时更保守，落后时更激进
    if (player->your_score > player->opponent_score + 500 && player->your_status == 0) {
        // 领先时，优先选择最安全的方向
        int bestX = currentX, bestY = currentY, maxDist = -1;
        int dx[] = {-1, 1, 0, 0};
        int dy[] = {0, 0, -1, 1};
    ...
    if (player->your_score + 500 < player->opponent_score) {
        // 落后时，优先抢分，适度冒险
        if (findNearestSuperStar(player, currentX, currentY, &target)) {
            if (findPath(player, currentX, currentY, target.X, target.Y, &nextMove)) {
                return nextMove;
